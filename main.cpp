...
vector<Ghost*> ghosts;
vector<vector<float>> positionVector = {{10.5, 8.5, 1.0}, {13.5, 1.5, 2.0}, {4.5, 6.5, 3.0}, {2.5, 13.5, 4.0}};

Ghost* monster1 = new Ghost(10.5, 8.5, 1.0); //coordinates and direction of first monster
Ghost* monster2 = new Ghost(13.5, 1.5, 2.0); //coordinates and direction of second monster
Ghost* monster3 = new Ghost(4.5, 6.5, 3.0); //coordinates and direction of third monster
Ghost* monster4 = new Ghost(2.5, 13.5, 4.0); //coordinates and direction of fourth monster
...
void init(void){
    ...
    ghosts.push_back(monster1);
    ghosts.push_back(monster2);
    ghosts.push_back(monster3);
    ghosts.push_back(monster4);
...
void createGhost()
{
    int temp = (int)ghosts.size();
    for (int i = 0; i < temp; i++)
    {
        int row = rand() % 4;
        Ghost* ghost = new Ghost(positionVector[row][0], positionVector[row][1], positionVector[row][2]);
        ghosts.push_back(ghost);
        numberOfGhosts++;
    }
}

void drawMonster(Ghost* monster , float r, float g, float b){
    int x, y;
    glBegin(GL_LINES);
    glColor3f(r, g, b);
    //draw the head
    for (int k = 0; k < 32; k++){
        x = (float)k / 2.0 * cos(360 * M_PI / 180.0) + (monster->posX*squareSize);
        y = (float)k / 2.0* sin(360 * M_PI / 180.0) + (monster->posY*squareSize);
        for (int i = 180; i <= 360; i++){
            glVertex2f(x, y);
            x = (float)k / 2.0 * cos(i * M_PI / 180.0) + (monster->posX*squareSize);
            y = (float)k / 2.0* sin(i * M_PI / 180.0) + (monster->posY*squareSize);
            glVertex2f(x, y);
        }
    }
    glEnd();
    //draw body
    glRectf((monster->posX*squareSize) - 17, monster->posY*squareSize, (monster->posX*squareSize) + 15, (monster->posY*squareSize) + 15);
    glBegin(GL_POINTS);
    glColor3f(0, 0.2, 0.4);
    //draw eyes and legs
    glVertex2f((monster->posX*squareSize) - 11, (monster->posY*squareSize) + 14); //legs
    glVertex2f((monster->posX*squareSize) - 1, (monster->posY*squareSize) + 14); //legs
    glVertex2f((monster->posX*squareSize) + 8, (monster->posY*squareSize) + 14); //legs
    glVertex2f((monster->posX*squareSize) + 4, (monster->posY*squareSize) - 3); //eyes
    glVertex2f((monster->posX*squareSize) - 7, (monster->posY*squareSize) - 3); //eyes
    glEnd();
}

//Method to update the position of the monsters randomly
void updateMonster(Ghost* monster){
        //find the current position of the monster
        int x1Quadrant = (int)((monster->posX - (2/squareSize)) - (16.0 *cos(360 * M_PI / 180.0)) / squareSize);
        int x2Quadrant = (int)((monster->posX + (2/squareSize)) + (16.0 *cos(360 * M_PI / 180.0)) / squareSize);
        int y1Quadrant = (int)((monster->posY - (2/squareSize)) - (16.0 *cos(360 * M_PI / 180.0)) / squareSize);
        int y2Quadrant = (int)((monster->posY + (2/squareSize)) + (16.0 *cos(360 * M_PI / 180.0)) / squareSize);
        //move him acording to its direction until he hits an obstacle
        switch ((int)monster->direction){
        case 1:
            if (!bitmap.at(x1Quadrant).at((int)monster->posY)){
                monster->posX -= 2 / squareSize;
            }else {
            ...
            break;
        case 2:
            if (!bitmap.at(x2Quadrant).at((int)monster->posY)){
                monster->posX += 2 / squareSize;
            }
            else {
                int current = monster->direction;
            ...
            break;
        case 3:
            if (!bitmap.at((int)monster->posX).at(y1Quadrant)){
                monster->posY -= 2 / squareSize;
            }
            else {
                int current = monster->direction;
                do{
            ...
            break;
        case 4:
            if (!bitmap.at((int)monster->posX).at(y2Quadrant)){
                monster->posY += 2 / squareSize;
            }
            else {
                int current = monster->direction;
...
void gameOver(){
    ...
    {
        int ghostX = (int)(ghosts[i]->posX);
        int ghostY = (int)(ghosts[i]->posY);
    ...
    if (replay){
        if (!over){
            ...
            for (int i = 0; i < ghosts.size(); i++)
            {
                ...
                //ghosts.at(i)->updateMonster(bitmap);
                //ghosts.at(i)->drawMonster(0.0, 1.0, 1.0);
                updateMonster(ghost);
    //            updateMonster(monster2, 2);
    //            updateMonster(monster3, 3);
    //            updateMonster(monster4, 4);
                drawMonster(ghost, 0.0, 1.0, 1.0); //cyan
